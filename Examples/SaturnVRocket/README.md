...
- from the current acceleration, calculate the change in the velocity;
- from the current velocity, calculate the change in the height;
- from the rate at which fuel is used up, decrease the remaining mass of the fuel.
...
(assuming that the rocket gets off the ground at all!!! - need to check for this unfortunate situation!). Here, we have assumed that each iteration lasts 1 second; the actual time-step could be set to something else if we multiple these things by it.
